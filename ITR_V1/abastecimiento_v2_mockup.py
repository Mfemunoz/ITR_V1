...
col_logo, col_title, col_status = st.columns([1.2, 3.0, 1.8])
with col_logo:
    if logo:
        st.image(logo, width=170)
    else:
        st.markdown("<h2 style='color:#ed1c24;'>SODIMAC</h2>", unsafe_allow_html=True)
with col_title:
    st.markdown(
        "<h1 style='color:#ed1c24;margin-bottom:0;'>SODIMAC COLOMBIA - DESARROLLO LOGISTICO</h1>"
        "<h3 style='color:#004b8d;margin-top:0;border-left:8px solid #ffdd00;padding-left:10px;'>Centro de Control de Abastecimiento</h3>",
        unsafe_allow_html=True
    )
with col_status:
    if secondary_logo:
...
with st.sidebar:
    ...
    selected = option_menu(
        ...
        styles={
            ...
            'nav-link': {'font-size': '14px', 'text-align': 'left', 'margin': '2px 0', '--hover-color': '#eef6fc'},
            'nav-link-selected': {'background-color': '#004b8d', 'color': 'white'},
        }
    )
...
if selected == 'Ejecución Abastecimiento':
    ...
    if df is None:
        ...
        with tab_indicadores:
            ...
            mapa_tiendas = preparar_mapa_tiendas(df)
            mapa_ok = mapa_tiendas.dropna(subset=['Latitud', 'Longitud'])
            st.markdown('Top 10 tiendas del piloto en mapa de Colombia')
            if mapa_ok.empty:
