...
def generar_proyeccion_transporte(df_base, excel_source, fecha_despacho, ton_pallet_default, costo_km_minimula, costo_km_tractomula, solo_itr=True):
    ...
    return pd.DataFrame(viajes), messages
...
if not st.session_state.authenticated:
    ...
    st.subheader('Modelo Transporte - Consolidación ITR')
    if df is None:
        st.markdown(
            "<div class='empty-state'><div class='empty-title'>Modelo pendiente de archivo</div>"
            "<div class='empty-text'>Carga el archivo operativo para proyectar viajes, tipología vehicular, costos y trazabilidad.</div></div>",
            unsafe_allow_html=True
        )
    else:
        st.markdown(
            "<div class='empty-state'><div class='empty-title'>Reglas del modelo</div>"
            ...
            "Cuando el viaje queda en 15 toneladas o menos se marca como no viable por baja consolidación.</div></div>",
            unsafe_allow_html=True
        )
        ...
        proyeccion, transporte_messages = generar_proyeccion_transporte(
            df,
            excel_source,
            fecha_despacho,
            ton_pallet_default,
        ...
        if proyeccion.empty:
            st.warning('No hay registros disponibles para proyectar transporte con los filtros actuales.')
        else:
            ...
            metrics = [
                ('Viajes proyectados', f'{len(vista_transporte):,}'),
                ('Viajes viables', f'{viable_mask.sum():,}'),
                ('No viables', f'{(~viable_mask).sum():,}'),
            ...
            with g1:
                ...
                if not costo_chart.empty:
                    ...
                    fig_costo = aplicar_tema_grafico(px.bar(
                        ...
                        color='Escenario',
                        barmode='group',
                        title='Comparativo de costo por viaje'
                    ))
            ...
            with g2:
                ...
                fig_viable = aplicar_tema_grafico(px.pie(
                    ...
                    names='Viabilidad',
                    values='Viajes',
                    title='Viabilidad de consolidación',
                    color_discrete_sequence=SODIMAC_COLORS
                ))
                st.plotly_chart(fig_viable, use_container_width=True)

            st.dataframe(
                vista_transporte.sort_values(['Viabilidad', 'Region', 'Toneladas'], ascending=[False, True, False]),
                use_container_width=True
            )
            st.download_button(
                'Exportar modelo transporte',
                to_excel(vista_transporte),
                'modelo_transporte_itr.xlsx',
                mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
            )
