...
@st.cache_data(show_spinner=False)
def load_planeacion_itr(uploaded_file):
    uploaded_file.seek(0)
    excel = pd.ExcelFile(uploaded_file, engine='openpyxl')
...
with st.sidebar:
    ...
    selected = option_menu(
        'Centro de Control',
        ['Inicio', 'Consulta Individual', 'Consulta Masiva', 'Dashboard Ejecutivo', 'Planeación ITR', 'TMS', 'Auditoría', 'Reportes'],
        icons=['house', 'search', 'upload', 'bar-chart', 'calendar-week', 'truck', 'clipboard-data', 'file-earmark-arrow-down'],
        default_index=0
    )
...
if uploaded_file:
    try:
        ...
        planeacion_df, planeacion_messages = load_planeacion_itr(uploaded_file)
        tms_df, tms_messages = load_tms_data(uploaded_file)
