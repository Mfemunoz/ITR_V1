...
ITR_BUENAVENTURA = {'Punto': 'ITR Buenaventura', 'Latitud': 3.8801, 'Longitud': -77.0312}
CD_MADRID = {'Punto': 'CD Madrid', 'Latitud': 4.7351, 'Longitud': -74.2646}
...
@st.cache_data(show_spinner=False)
def load_referencias_transporte(excel_source):
    if source_is_csv(excel_source):
        return {}, {}, ['CSV cargado: se usará la tonelada por pallet por defecto y la región quedará como no definida.']

    excel_source = reset_excel_source(excel_source)
    excel = pd.ExcelFile(excel_source, engine='openpyxl')
    if 'Transformacion a Palets' not in excel.sheet_names:
        return {}, {}, ['No existe la hoja Transformacion a Palets. Se usará la tonelada por pallet por defecto.']

    ref = pd.read_excel(excel, sheet_name='Transformacion a Palets')
    ref.columns = ref.columns.astype(str).str.strip()
    producto_ton_pallet = {}
    tienda_region = {}

    if {'Producto', 'Toneladas', 'Calculo Palets'}.issubset(ref.columns):
        productos = ref[['Producto', 'Toneladas', 'Calculo Palets']].dropna(subset=['Producto']).copy()
        productos['Toneladas'] = pd.to_numeric(productos['Toneladas'], errors='coerce')
        productos['Calculo Palets'] = pd.to_numeric(productos['Calculo Palets'], errors='coerce')
        productos = productos[(productos['Toneladas'] > 0) & (productos['Calculo Palets'] > 0)]
        productos['Toneladas_Por_Palet'] = productos['Toneladas'] / productos['Calculo Palets']
        producto_ton_pallet = dict(zip(productos['Producto'].map(normalizar_texto), productos['Toneladas_Por_Palet']))

    if {'COD TIENDA', 'REGION'}.issubset(ref.columns):
        tiendas = ref[['COD TIENDA', 'REGION']].dropna(subset=['COD TIENDA']).copy()
        tiendas['COD TIENDA'] = pd.to_numeric(tiendas['COD TIENDA'], errors='coerce')
        tiendas = tiendas.dropna(subset=['COD TIENDA'])
        tiendas['COD TIENDA'] = tiendas['COD TIENDA'].astype(int)
        tiendas['REGION'] = tiendas['REGION'].fillna('REGION NO DEFINIDA').astype(str).str.strip()
        tienda_region = dict(zip(tiendas['COD TIENDA'], tiendas['REGION']))

    messages = []
    if not producto_ton_pallet:
        messages.append('No se encontraron toneladas por producto en Transformacion a Palets.')
    if not tienda_region:
        messages.append('No se encontró relación tienda-region en Transformacion a Palets.')
    return producto_ton_pallet, tienda_region, messages
...
def ruta_minima_km(origen, tiendas):
    coords = []
    for tienda in tiendas:
        coord = STORE_COORDS.get(int(tienda))
        if coord:
            coords.append((int(tienda), coord[0], coord[1]))
    if len(coords) != len(tiendas):
        return None
    if len(coords) == 1:
        return distancia_km(origen['Latitud'], origen['Longitud'], coords[0][1], coords[0][2])

    primero, segundo = coords[0], coords[1]
    ruta_a = (
        distancia_km(origen['Latitud'], origen['Longitud'], primero[1], primero[2])
        + distancia_km(primero[1], primero[2], segundo[1], segundo[2])
    )
    ruta_b = (
        distancia_km(origen['Latitud'], origen['Longitud'], segundo[1], segundo[2])
        + distancia_km(segundo[1], segundo[2], primero[1], primero[2])
    )
    return min(ruta_a, ruta_b)


def clasificar_vehiculo(toneladas):
    if toneladas <= 15:
        return 'SIN VEHICULO', 'NO VIABLE - BAJA CONSOLIDACION'
    if toneladas < 20:
        return 'MINIMULA / PATINETA', 'VIABLE'
    return 'TRACTOMULA', 'VIABLE'


def codigo_region(region):
    limpio = ''.join(ch for ch in normalizar_texto(region) if ch.isalnum())
    return (limpio[:3] or 'REG')


def generar_proyeccion_transporte(df_base, excel_source, fecha_despacho, ton_pallet_default, costo_km_minimula, costo_km_tractomula, solo_itr=True):
    producto_ton_pallet, tienda_region, messages = load_referencias_transporte(excel_source)
    base = df_base.copy()
    if solo_itr:
        base = base[base['Decision_Python'].str.contains('SURTIR DESDE ITR', na=False)].copy()
    else:
        base = base[base['Decision_Python'].str.contains('SURTIR DESDE', na=False)].copy()

    if base.empty:
        return pd.DataFrame(), pd.DataFrame(), messages

    base['Producto_Key'] = base['DESC SKU'].map(normalizar_texto)
    base['Toneladas_Por_Palet'] = base['Producto_Key'].map(producto_ton_pallet).fillna(ton_pallet_default)
    base['Toneladas_Estimadas'] = base['Proyectado_Palets'] * base['Toneladas_Por_Palet']
    base['Region'] = base['Tienda'].map(tienda_region).fillna('REGION NO DEFINIDA')

    tiendas = (
        base.groupby(['Region', 'Tienda', 'Nombre Tienda'], as_index=False)
        .agg(
            Toneladas=('Toneladas_Estimadas', 'sum'),
            Palets=('Proyectado_Palets', 'sum'),
            SKU_Count=('SKU', 'nunique'),
            SKUs=('SKU', lambda s: ', '.join(map(str, sorted(set(s))[:12]))),
        )
        .sort_values(['Region', 'Toneladas'], ascending=[True, False])
    )

    viajes = []
    detalle_viajes = []
    secuencia = 1
    for region, grupo in tiendas.groupby('Region'):
        pendientes = grupo.to_dict('records')
        pendientes.sort(key=lambda r: r['Toneladas'], reverse=True)

        while pendientes:
            actual = pendientes.pop(0)
            seleccion = [actual]
            partner_idx = None
            mejor_total = actual['Toneladas']

            if actual['Toneladas'] < 25 and pendientes:
                for idx, candidato in enumerate(pendientes):
                    total = actual['Toneladas'] + candidato['Toneladas']
                    if total <= 25 and total > mejor_total:
                        partner_idx = idx
                        mejor_total = total
                if partner_idx is not None:
                    seleccion.append(pendientes.pop(partner_idx))

            tiendas_viaje = [int(s['Tienda']) for s in seleccion]
            nombres_tiendas = [str(s['Nombre Tienda']) for s in seleccion]
            toneladas = sum(float(s['Toneladas']) for s in seleccion)
            palets = sum(float(s['Palets']) for s in seleccion)
            sku_count = sum(int(s['SKU_Count']) for s in seleccion)
            skus = ', '.join([str(s['SKUs']) for s in seleccion if str(s['SKUs']).strip()])
            tipo_vehiculo, viabilidad = clasificar_vehiculo(toneladas)
            motivo_no_viable = '' if viabilidad == 'VIABLE' else 'No viable realizar despacho directo desde ITR por baja consolidacion de mercancia.'
            estado_orden = 'ORDEN DE DESPACHO SUGERIDA' if viabilidad == 'VIABLE' else 'ORDEN NO VIABLE'
            costo_km = costo_km_tractomula if tipo_vehiculo == 'TRACTOMULA' else costo_km_minimula

            km_directo = ruta_minima_km(ITR_BUENAVENTURA, tiendas_viaje)
            km_madrid_tienda = ruta_minima_km(CD_MADRID, tiendas_viaje)
            km_itr_madrid = distancia_km(
                ITR_BUENAVENTURA['Latitud'], ITR_BUENAVENTURA['Longitud'],
                CD_MADRID['Latitud'], CD_MADRID['Longitud']
            )
            km_via_cd = (km_itr_madrid + km_madrid_tienda) if km_madrid_tienda is not None else None

            es_viable = viabilidad == 'VIABLE'
            costo_directo = (km_directo * costo_km) if es_viable and km_directo is not None else 0
            costo_via_cd = (km_via_cd * costo_km) if es_viable and km_via_cd is not None else 0
            diferencia = costo_via_cd - costo_directo if es_viable else 0

            codigo_seguimiento = f'TRK-{fecha_despacho.strftime("%Y%m%d")}-{codigo_region(region)}-{secuencia:03d}'
            viajes.append({
                'Codigo_Seguimiento': codigo_seguimiento,
                'Fecha_Despacho': fecha_despacho,
                'Estado_Orden': estado_orden,
                'Region': region,
                'Tiendas': ' / '.join(map(str, tiendas_viaje)),
                'Nombres_Tiendas': ' / '.join(nombres_tiendas),
                'SKU_Count': sku_count,
                'SKUs': skus,
                'Palets': round(palets, 1),
                'Toneladas': round(toneladas, 2),
                'Tipo_Vehiculo': tipo_vehiculo,
                'Viabilidad': viabilidad,
                'Motivo_No_Viable': motivo_no_viable,
                'Km_Directo_ITR': round(km_directo, 1) if km_directo is not None else None,
                'Costo_Directo_ITR': round(costo_directo, 0),
                'Km_Via_CD_Madrid': round(km_via_cd, 1) if km_via_cd is not None else None,
                'Costo_Via_CD_Madrid': round(costo_via_cd, 0),
                'Ahorro_Directo_vs_CD': round(diferencia, 0),
            })
            detalle_base = base[(base['Region'].eq(region)) & (base['Tienda'].isin(tiendas_viaje))].copy()
            detalle = (
                detalle_base.groupby(['Tienda', 'Nombre Tienda', 'SKU', 'DESC SKU'], as_index=False)
                .agg(
                    Palets_Sugeridos=('Proyectado_Palets', 'sum'),
                    Toneladas_Estimadas=('Toneladas_Estimadas', 'sum'),
                    Decision_Python=('Decision_Python', 'first'),
                    Calendario=('CALENDAR', 'first'),
                )
                .sort_values(['Tienda', 'SKU'])
            )
            detalle.insert(0, 'Codigo_Seguimiento', codigo_seguimiento)
            detalle.insert(1, 'Fecha_Despacho', fecha_despacho)
            detalle.insert(2, 'Region', region)
            detalle['Tipo_Vehiculo'] = tipo_vehiculo
            detalle['Viabilidad'] = viabilidad
            detalle['Palets_Sugeridos'] = detalle['Palets_Sugeridos'].round(1)
            detalle['Toneladas_Estimadas'] = detalle['Toneladas_Estimadas'].round(2)
            detalle_viajes.append(detalle)
            secuencia += 1

    detalle_viajes = pd.concat(detalle_viajes, ignore_index=True) if detalle_viajes else pd.DataFrame()
    return pd.DataFrame(viajes), detalle_viajes, messages
...
def to_excel_orden_transporte(resumen, detalle):
    output = BytesIO()
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        resumen.to_excel(writer, index=False, sheet_name='Resumen_Ordenes')
        detalle.to_excel(writer, index=False, sheet_name='Detalle_SKU')
    return output.getvalue()
...
with st.sidebar:
    ...
    selected = option_menu(
        None,
        ['Planeación ITR', 'Modelo Transporte', 'Ejecución Abastecimiento', 'TMS', 'Consultas', 'Reportes'],
        icons=['calendar-week', 'truck-front', 'box-seam', 'truck', 'search', 'file-earmark-arrow-down'],
        default_index=0,
        styles={
...
elif selected == 'Modelo Transporte':
    st.subheader('Modelo Transporte - Orden de Despacho Sugerida')
    if df is None:
        st.markdown(
            "<div class='empty-state'><div class='empty-title'>Orden pendiente de archivo</div>"
            "<div class='empty-text'>Carga el archivo operativo para generar órdenes sugeridas, tipología vehicular, costos y trazabilidad.</div></div>",
            unsafe_allow_html=True
        )
    else:
        st.markdown(
            "<div class='empty-state'><div class='empty-title'>Reglas de generación de orden</div>"
            "<div class='empty-text'>Consolida SKU por región con máximo dos tiendas por viaje. "
            "Minimula/Patineta aplica para más de 15 y menos de 20 toneladas; Tractomula aplica desde 20 toneladas. "
            "Cuando el viaje queda en 15 toneladas o menos se marca como no viable por baja consolidación. "
            "La salida genera un resumen de orden y el detalle de SKU/pallets por tienda.</div></div>",
            unsafe_allow_html=True
        )

        c1, c2, c3, c4 = st.columns(4)
        with c1:
            fecha_despacho = st.date_input('Fecha despacho', value=datetime.now().date())
        with c2:
            ton_pallet_default = st.number_input('Ton por pallet sin referencia', min_value=0.1, max_value=5.0, value=1.0, step=0.1)
        with c3:
            costo_km_minimula = st.number_input('Costo km Minimula/Patineta', min_value=0, value=7500, step=500)
        with c4:
            costo_km_tractomula = st.number_input('Costo km Tractomula', min_value=0, value=9500, step=500)

        solo_itr = st.toggle('Usar solo decisiones SURTIR DESDE ITR', value=True)
        generar_orden = st.button('Generar orden de despacho', use_container_width=True)
        proyeccion, detalle_sku, transporte_messages = generar_proyeccion_transporte(
            df,
            excel_source,
            fecha_despacho,
            ton_pallet_default,
        ...
        for msg in transporte_messages:
            st.info(msg)

        if proyeccion.empty:
            st.warning('No hay registros disponibles para generar órdenes con los filtros actuales.')
        else:
            if generar_orden:
                st.success(f'Ordenes sugeridas generadas: {len(proyeccion):,} viajes proyectados y {len(detalle_sku):,} líneas de SKU.')
            region_options = sorted([r for r in proyeccion['Region'].dropna().astype(str).unique() if r])
            vehiculo_options = sorted([v for v in proyeccion['Tipo_Vehiculo'].dropna().astype(str).unique() if v])
            viabilidad_options = sorted([v for v in proyeccion['Viabilidad'].dropna().astype(str).unique() if v])
            f1, f2, f3, f4 = st.columns(4)
            with f1:
                region_filtro = st.multiselect('Región', region_options, default=region_options)
            with f2:
                vehiculo_filtro = st.multiselect('Tipo vehículo', vehiculo_options, default=vehiculo_options)
            with f3:
                viabilidad_filtro = st.multiselect('Viabilidad', viabilidad_options, default=viabilidad_options)
            with f4:
                buscar_transporte = st.text_input('Buscar tienda o seguimiento')

            vista_transporte = proyeccion.copy()
            if region_filtro:
                vista_transporte = vista_transporte[vista_transporte['Region'].astype(str).isin(region_filtro)]
            if vehiculo_filtro:
                vista_transporte = vista_transporte[vista_transporte['Tipo_Vehiculo'].astype(str).isin(vehiculo_filtro)]
            if viabilidad_filtro:
                vista_transporte = vista_transporte[vista_transporte['Viabilidad'].astype(str).isin(viabilidad_filtro)]
            if buscar_transporte:
                q = buscar_transporte.strip().upper()
                vista_transporte = vista_transporte[
                    vista_transporte['Codigo_Seguimiento'].astype(str).str.upper().str.contains(q, na=False)
                    | vista_transporte['Tiendas'].astype(str).str.upper().str.contains(q, na=False)
                    | vista_transporte['Nombres_Tiendas'].astype(str).str.upper().str.contains(q, na=False)
                ]
            detalle_filtrado = detalle_sku[detalle_sku['Codigo_Seguimiento'].isin(vista_transporte['Codigo_Seguimiento'])].copy()

            viable_mask = vista_transporte['Viabilidad'].eq('VIABLE')
            metrics = [
                ('Ordenes generadas', f'{len(vista_transporte):,}'),
                ('Ordenes viables', f'{viable_mask.sum():,}'),
                ('Ordenes no viables', f'{(~viable_mask).sum():,}'),
                ('Toneladas', f'{vista_transporte["Toneladas"].sum():,.1f}'),
                ('Palets', f'{vista_transporte["Palets"].sum():,.0f}'),
                ('Costo directo ITR', f'${vista_transporte.loc[viable_mask, "Costo_Directo_ITR"].sum():,.0f}'),
                ('Costo vía CD', f'${vista_transporte.loc[viable_mask, "Costo_Via_CD_Madrid"].sum():,.0f}'),
                ('Ahorro directo', f'${vista_transporte.loc[viable_mask, "Ahorro_Directo_vs_CD"].sum():,.0f}'),
                ('Tractomulas', f'{vista_transporte["Tipo_Vehiculo"].eq("TRACTOMULA").sum():,}'),
                ('Minimulas', f'{vista_transporte["Tipo_Vehiculo"].eq("MINIMULA / PATINETA").sum():,}'),
            ]
            pintar_metricas(metrics)
            st.divider()
            ...
            g1, g2 = st.columns(2)
            with g1:
                costo_chart = vista_transporte[vista_transporte['Viabilidad'].eq('VIABLE')].copy()
                if not costo_chart.empty:
                    costo_long = costo_chart.melt(
                        id_vars=['Codigo_Seguimiento'],
                        value_vars=['Costo_Directo_ITR', 'Costo_Via_CD_Madrid'],
                        var_name='Escenario',
                        value_name='Costo'
                    )
                    fig_costo = aplicar_tema_grafico(px.bar(
                        costo_long,
                        x='Codigo_Seguimiento',
                        y='Costo',
                        color='Escenario',
                        barmode='group',
                        title='Comparativo de costo por orden'
                    ))
                    st.plotly_chart(fig_costo, use_container_width=True)
                else:
                    st.info('No hay viajes viables para comparar costos.')
            with g2:
                viable_chart = vista_transporte['Viabilidad'].value_counts().reset_index()
                viable_chart.columns = ['Viabilidad', 'Viajes']
                fig_viable = aplicar_tema_grafico(px.pie(
                    viable_chart,
                    names='Viabilidad',
                    values='Viajes',
                    title='Viabilidad de orden sugerida',
                    color_discrete_sequence=SODIMAC_COLORS
                ))
                st.plotly_chart(fig_viable, use_container_width=True)

            st.markdown('Resumen de órdenes sugeridas')
            columnas_resumen = [
                'Codigo_Seguimiento', 'Fecha_Despacho', 'Estado_Orden', 'Region', 'Tiendas',
                'Nombres_Tiendas', 'SKU_Count', 'Palets', 'Toneladas', 'Tipo_Vehiculo',
                'Viabilidad', 'Motivo_No_Viable', 'Km_Directo_ITR', 'Costo_Directo_ITR',
                'Km_Via_CD_Madrid', 'Costo_Via_CD_Madrid', 'Ahorro_Directo_vs_CD'
            ]
            st.dataframe(
                vista_transporte[columnas_resumen].sort_values(['Viabilidad', 'Region', 'Toneladas'], ascending=[False, True, False]),
                use_container_width=True
            )

            st.markdown('Detalle por SKU y tienda')
            columnas_detalle = [
                'Codigo_Seguimiento', 'Fecha_Despacho', 'Region', 'Tienda', 'Nombre Tienda',
                'SKU', 'DESC SKU', 'Palets_Sugeridos', 'Toneladas_Estimadas',
                'Tipo_Vehiculo', 'Viabilidad', 'Decision_Python', 'Calendario'
            ]
            st.dataframe(
                detalle_filtrado[columnas_detalle].sort_values(['Codigo_Seguimiento', 'Tienda', 'SKU']),
                use_container_width=True
            )
            st.download_button(
                'Exportar orden de despacho',
                to_excel_orden_transporte(vista_transporte[columnas_resumen], detalle_filtrado[columnas_detalle]),
                'orden_despacho_sugerida_itr.xlsx',
                mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
            )
